...
TEST(LookupTableTest, TestTransaxialLookup) {
  ...
  auto det_pair = LUT->lookup(0, 0, 0); // Horizontal through center

  EXPECT_EQ(det_pair.first.ring_idx, 0);
  EXPECT_EQ(det_pair.second.ring_idx, DETECTORS_PER_RING / 2);

  det_pair = LUT->lookup(1, 0, 0); // Tilted one bin through center

  EXPECT_EQ(det_pair.first.ring_idx, 1);
  EXPECT_EQ(det_pair.second.ring_idx, 253);
  ...
  // and angle number
  EXPECT_EQ(det_pair.first.ring_idx, 1);
  EXPECT_EQ(det_pair.second.ring_idx, 251);
  ...
  EXPECT_EQ(det_pair.first.ring_idx, 503);
  EXPECT_EQ(det_pair.second.ring_idx, 253);
  ...
  EXPECT_EQ(det_pair.first.ring_idx, 419);
  EXPECT_EQ(det_pair.second.ring_idx, 337);

  det_pair = LUT->lookup(251, 0, 0);

  EXPECT_EQ(det_pair.first.ring_idx, 251);
  EXPECT_EQ(det_pair.second.ring_idx, 503);
