...
LOR::get_det_positions(const ScannerGeometry &geometry) {
  ...
  // Line is horizontal
  if (angle_idx == geometry.det_per_ring / 4) {
    result.first.x = (geometry.detector_diameter / 2) -
                     (0.5 + tang_idx) * geometry.voxel_size;
    result.second.x = result.first.x;
    result.first.y = sqrt(geometry.detector_diameter * (0.5 + tang_idx) *
                              geometry.voxel_size -
                          pow((0.5 + tang_idx) * geometry.voxel_size, 2));
